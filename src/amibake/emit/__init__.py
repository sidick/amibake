...
def collect_emulator_config(plan, library: dict, emitter: str) -> dict:
    ...
    on key conflict, then the manifest's own `[emulator-config.<emitter>]`
    last of all — the manifest describes the test setup, so it outranks
    any recipe. Recipes with no directives for this emitter simply
    contribute nothing."""
    merged: dict = {}
    ...
    merged.update(plan.emulator_config.get(emitter) or {})
    return merged
